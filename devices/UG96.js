/*
  ...
  Author: Thierry Biniguer (thierry.biniguer@st.com) for STMicroelectronics
...
/* supported features
	...
	-	Flow control, as Espruino implementation :
		Espruino sets the CTS pin whenever it is ready to receive data or whenever it is too busy to receive
	- full modem initialization : AT synchronization, SIM card detection, signal measurement, automatic operator selection
...
var netCallbacks = {
  ...
  recv: function(sckt, maxLen) {
    ...
    if (sockData[sckt]) {
      var r;
	  //if (dbg) console.log("receiving data in socket, length:"+sockData[sckt].length+"(max len=)"+maxLen);
  ...
  send: function(sckt, data) {
	...
	//if (dbg) console.log("sending data :" + data);
...
function receiveHandler(line) {
  ...
  // parms[0] is for the connectID
  // parms[1] is for the currentrecvlength
  //if (dbg) console.log(parms[0]);
  //if (dbg) console.log(parms[1]);

  if (len>=parms[1]) {
   // we have everything
	sockData[parms[0]] += line.substr(colon+2,parms[1]);
	//if (dbg) console.log("All data are digested:"+sockData[parms[0]] );
	//if (dbg) console.log("Data Left behind:");
	//if (dbg) console.log(line.substr(colon+parms[1]+3));

	return line.substr(colon+parms[1]+3); // return anything else
  } else {
	// still some to get
	sockData[parms[0]] += line.substr(colon+2,len);
	//if (dbg) console.log("Partial data stored:"+sockData[parms[0]]);
	//if (dbg) console.log("receiveHandler out2in:");
	//if (dbg) console.log("+D,"+parms[0]+","+(parms[1]-len)+":");

   return "+D,"+parms[0]+","+(parms[1]-len)+":"; // return +D so receiveHandler2 gets called next time
  }
}
function receiveHandler2(line) {
  /*if (dbg) console.log("receiveHandler2 line in:");
  if (dbg) console.log(line);*/
  ...
  if (len>=parms[1]) {
   ...
   //if (dbg) console.log("All data recomposed:"+sockData[parms[0]]);
   //if (dbg) console.log("Data Left behind:"+ line.substr(colon+parms[1]+1));
   return line.substr(colon+parms[1]+1); // return anything else
  } else {
   // still some to get
   sockData[parms[0]] += line.substr(colon+1,len);
   //if (dbg) console.log("Partial data stored:"+sockData[parms[0]]);
   //if (dbg) console.log("receiveHandler2 out2in:");
   //if (dbg) console.log("+D,"+parms[0]+","+(parms[1]-len)+":");
   return "+D,"+parms[0]+","+(parms[1]-len)+":"; // return +D so receiveHandler2 gets called next time
  }
}
